...
if [ "$1" = "vit_b" ]; then
    python preprocess.py --model-type vit_b --checkpoint-path "$PRETRAIN_DIR/sam_vit_b_01ec64.pth"
elif [ "$1" = "vit_l" ]; then
    python preprocess.py --model-type vit_l --checkpoint-path "$PRETRAIN_DIR/sam_vit_l_0b3195.pth"
else
    python preprocess.py --model-type vit_h --checkpoint-path "$PRETRAIN_DIR/sam_vit_h_4b8939.pth"
fi
